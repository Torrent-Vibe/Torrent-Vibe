...
WORK_DIR="$(mktemp -d /tmp/torrent-vibe-desktop-release-dry-run.XXXXXX)"
ACCOUNT="torrent-vibe-dryrun-$$-$RANDOM"
RAM_DEV=""
...
cleanup() {
  ...
  # electron-builder.yml and package.json are restored even on failure so a dry
  # run never leaves a real key or a broken placeholder in the working tree.
  git -C "$REPO_DIR" checkout -- electron-builder.yml package.json 2>/dev/null || true
}
trap cleanup EXIT
...
if grep -q "$PLACEHOLDER" "$REPO_DIR/electron-builder.yml"; then
  sed -i '' "s|$PLACEHOLDER|$PUBLIC_KEY|" "$REPO_DIR/electron-builder.yml"
...
const p = '$REPO_DIR/package.json';
const pkg = JSON.parse(fs.readFileSync(p, 'utf8'));
pkg.version = '$OLD_VERSION';
fs.writeFileSync(p, JSON.stringify(pkg, null, 2) + '\n');
"
rm -rf "$HOME/Library/Caches/electron-rebuild"
( cd "$REPO_DIR" && pnpm build:electron )
OLD_ZIP_PATH="$(ls "$REPO_DIR"/release/*.zip)"
verify_zip "$OLD_ZIP_PATH"
mkdir -p "$WORK_DIR/archive"
cp "$OLD_ZIP_PATH" "$WORK_DIR/archive/"
rm -rf "$REPO_DIR/release"

log "== restore real version ($NEW_VERSION) and build the NEW build =="
node -e "
const fs = require('node:fs');
const p = '$REPO_DIR/package.json';
const pkg = JSON.parse(fs.readFileSync(p, 'utf8'));
...
rm -rf "$HOME/Library/Caches/electron-rebuild"
( cd "$REPO_DIR" && pnpm build:electron )
NEW_ZIP_PATH="$(ls "$REPO_DIR"/release/*.zip)"
NEW_DMG_PATH="$(ls "$REPO_DIR"/release/*.dmg)"
...
generate_appcast \
  ...
  --download-url-prefix "https://github.com/${REPO_SLUG}/releases/download/desktop-v${NEW_VERSION}/" \
  "$WORK_DIR/archive"
